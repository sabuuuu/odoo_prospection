...
def run_pipeline(dry_run: bool = False, limit_override: int = None):
    ...
    # 3. Parcours des critères de recherche
    for naf_code in Config.TARGET_NAF_CODES:
        dep_list = Config.TARGET_DEPARTMENTS if Config.TARGET_DEPARTMENTS else [None]
        for dep in dep_list:
            dep_display = f"Dept {dep}" if dep else "Toute France"
            logger.info(f"\n🔎 Recherche Pappers pour NAF: {naf_code} | Zone: {dep_display} (Limite: {limit})...")
            ...
            for company in companies:
                ...
                # 4. Dédoublonnage avec la base Odoo (vérifie dans les étapes spécifiées et globale)
                ...
                # Arrêt si le quota journalier est atteint
                if total_added >= limit:
                    logger.info(f"🎯 Quota journalier atteint ({limit} prospects ajoutés).")
                    break
